...
build_dist() {
  ...
  build_one() {
    ...
    GOOS=darwin GOARCH="${goarch}" CGO_ENABLED=0 \
      go build -trimpath \
        -ldflags "-s -w -X protonmailcli/internal/app.Version=${ver} -X protonmailcli/internal/app.Commit=${commit} -X protonmailcli/internal/app.Date=${date_utc}" \
        -o "${stage}/protonmailcli" \
        ./cmd/protonmailcli

    (cd "${stage}" && tar -czf "../protonmailcli_${ver}_darwin_${goarch}.tar.gz" protonmailcli)
    rm -rf "${stage}"
  }
...
update_homebrew_formula() {
  ...
  sha_arm="$(awk -v f="protonmailcli_${ver}_darwin_arm64.tar.gz" '$2==f{print $1}' dist/SHA256SUMS.txt | head -n 1 || true)"
  sha_amd="$(awk -v f="protonmailcli_${ver}_darwin_amd64.tar.gz" '$2==f{print $1}' dist/SHA256SUMS.txt | head -n 1 || true)"
  ...
  cat >"${tmp}/tap/Formula/protonmailcli.rb" <<RUBY
class Protonmailcli < Formula
  ...
  on_macos do
    on_arm do
      url "https://github.com/${repo_owner}/${repo_name}/releases/download/${ver}/protonmailcli_${ver}_darwin_arm64.tar.gz"
      sha256 "${sha_arm}"
    end
    on_intel do
      url "https://github.com/${repo_owner}/${repo_name}/releases/download/${ver}/protonmailcli_${ver}_darwin_amd64.tar.gz"
      sha256 "${sha_amd}"
    end
  end
  ...
  test do
    system "#{bin}/protonmailcli", "--version"
  end
end
RUBY
...
if "protonmailcli" not in formulae:
    formulae.append("protonmailcli")
    formulae.sort()
...
PY
  ...
  git add Formula/protonmailcli.rb README.md
  if git diff --cached --quiet; then
    print -- "Homebrew formula already up to date"
    popd >/dev/null
    return 0
  fi
  git commit -m "protonmailcli ${ver}"
